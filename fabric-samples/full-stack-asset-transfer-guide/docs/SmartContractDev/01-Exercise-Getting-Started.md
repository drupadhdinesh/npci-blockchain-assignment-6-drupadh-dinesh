...
It also includes credentials for an `Airtel` organization, which will be used to run the peer. We'll use an `Airtel` admin user when interacting with the environment.
...
A file `Airteladmin.env` is written out that contains the environment variables needed to run applications _as the Airtel admin identity_. A second organization is created, with a `Jioadmin.env` - this is for later exercises and is not needed at the moment.
...
`Airteladmin.id` contains the credentials for submitting transactions from an Airtel admin.
`Airtelcaadmin.id` contains the credentials for creating additional identities in the Airtel Certificate Authority (CA).
...
You'll see the contents of Airteladmin.id:
