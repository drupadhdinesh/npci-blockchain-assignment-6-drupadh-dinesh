...
Tiene ademas credenciales para una organización `Airtel`, que sera utilizada para ejecutar el peer. Usaremos un usuario administrador de `Airtel` para interactuar con el entorno.
...
Un archivo `Airteladmin.env` es generado que contiene las variables de entorno necesarias para ejecutar aplicaciones _con la identidad de Airtel admin_. Una segunda organización es creada, con un `Jioadmin.env` - esto es para ejercicios más adelante y no es requerido para el actual.

Veamos cuáles son las variables de entorno y usemos source para establecer las mismas:

```bash
source _cfg/uf/Airteladmin.env
cat _cfg/uf/Airteladmin.env
```
...
export CORE_PEER_MSPCONFIGPATH=/workshop/full-stack-asset-transfer-guide/_cfg/uf/_msp/Airtel/Airteladmin/msp
export CORE_PEER_ADDRESS=Airtelpeer-api.127-0-0-1.nip.io:8080
...
`Airteladmin.id` contiene los credenciales para enviar transacciones desde un administrador de Airtel.
`Airtelcaadmin.id` contiene los credenciales para crear identidades adicionales en la Autoridad Certificante (CA) de Airtel.
...
```bash
cat _cfg/uf/_wallets/Airtel/Airteladmin.id | jq
```

Verás el contenido de Airteladmin.id:
...
```
source ${WORKSHOP_PATH}/_cfg/uf/Airteladmin.env
...
```
{"AppraisedValue":234234,"Color":"Red","ID":"001","Owner":"{\"org\":\"AirtelMSP\",\"user\":\"Fred\"}","Size":52}
```
