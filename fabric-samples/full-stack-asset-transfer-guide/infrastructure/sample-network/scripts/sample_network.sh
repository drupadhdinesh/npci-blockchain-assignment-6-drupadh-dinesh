...
function network_up() {
  ...
  wait_for ibppeer Airtel-peer1
  wait_for ibppeer Airtel-peer2
  wait_for ibppeer Jio-peer1
  wait_for ibppeer Jio-peer2
...
function launch_network_CAs() {
  ...
  wait_for ibpca Airtel-ca
  wait_for ibpca Jio-ca
  ...
  export Airtel_CA_CERT=$(kubectl -n $NS get cm/Airtel-ca-connection-profile -o json | jq -r .binaryData.\"profile.json\" | base64 -d | jq -r .tls.cert)
  export Jio_CA_CERT=$(kubectl -n $NS get cm/Jio-ca-connection-profile -o json | jq -r .binaryData.\"profile.json\" | base64 -d | jq -r .tls.cert)

  enroll_bootstrap_rcaadmin org0 rcaadmin rcaadminpw
  enroll_bootstrap_rcaadmin Airtel rcaadmin rcaadminpw
  enroll_bootstrap_rcaadmin Jio rcaadmin rcaadminpw
