...
curl -s --cacert $WORKSHOP_CRYPTO/cas/Airtel-ca/tls-cert.pem https://$WORKSHOP_NAMESPACE-Airtel-ca-ca.$WORKSHOP_INGRESS_DOMAIN/cainfo | jq -c
curl -s --cacert $WORKSHOP_CRYPTO/cas/Jio-ca/tls-cert.pem https://$WORKSHOP_NAMESPACE-Jio-ca-ca.$WORKSHOP_INGRESS_DOMAIN/cainfo | jq -c
...
# Try a few times with Airtel-peer1
export ENDPOINT=${WORKSHOP_NAMESPACE}-Airtel-peer1-peer.${WORKSHOP_INGRESS_DOMAIN}:443
...
# Then with Airtel-peer2
export ENDPOINT=${WORKSHOP_NAMESPACE}-Airtel-peer2-peer.${WORKSHOP_INGRESS_DOMAIN}:443
...
# Then with the gateway endpoint.  Connections will be distributed across Airtel-peer1 and Airtel-peer2
