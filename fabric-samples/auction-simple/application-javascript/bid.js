...
const { buildCCPAirtel, buildCCPJio, buildWallet, prettyJSONString} = require('../../test-application/javascript/AppUtil.js');
...
async function main() {
	try {
		...
		if (org === 'Airtel' || org === 'Airtel') {
			...
			const orgMSP = 'AirtelMSP';
			const ccp = buildCCPAirtel();
			const walletPath = path.join(__dirname, 'wallet/Airtel');
			...
			const orgMSP = 'JioMSP';
			const ccp = buildCCPJio();
			const walletPath = path.join(__dirname, 'wallet/Jio');
			const wallet = await buildWallet(Wallets, walletPath);
			await bid(ccp,wallet,user,orgMSP,auctionID,price);
		}  else {
			console.log('Usage: node bid.js org userID auctionID price');
