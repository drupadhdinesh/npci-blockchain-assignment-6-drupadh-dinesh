...
class TokenERC20Contract extends Contract {
    ...
    */
    ...
    async Initialize(ctx, name, symbol, decimals) {
        // Check minter authorization - this sample assumes Airtel is the central banker with privilege to set Options for these tokens
        const clientMSPID = ctx.clientIdentity.getMSPID();
        if (clientMSPID !== 'AirtelMSP') {
    ...
    async Burn(ctx, amount) {
        ...
        // Check minter authorization - this sample assumes Airtel is the central banker with privilege to burn tokens
        const clientMSPID = ctx.clientIdentity.getMSPID();
        if (clientMSPID !== 'AirtelMSP') {
            throw new Error('client is not authorized to mint new tokens');
        }
