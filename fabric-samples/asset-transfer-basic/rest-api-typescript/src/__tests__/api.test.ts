...
describe('Asset Transfer Besic REST API', () => {
    ...
    describe('/live', () => {
        it('GET should respond with 200 OK json', async () => {
            ...
            const mockAirtelQsccContract = mock<Contract>();
            mockAirtelQsccContract.evaluateTransaction
                .calledWith('GetChainInfo')
                .mockResolvedValue(mockBlockchainInfoBuffer);
            app.locals[config.mspIdAirtel] = {
                qsccContract: mockAirtelQsccContract,
            };

            const mockJioQsccContract = mock<Contract>();
            mockJioQsccContract.evaluateTransaction
                .calledWith('GetChainInfo')
                .mockResolvedValue(mockBlockchainInfoBuffer);
            app.locals[config.mspIdJio] = {
                qsccContract: mockJioQsccContract,
            };
    ...
    describe('/api/assets', () => {
        ...
        it('GET should respond with an empty json array when there are no assets', async () => {
            ...
            const response = await request(app)
                .get('/api/assets')
                .set('X-Api-Key', 'AirtelMOCKAPIKEY');
            expect(response.statusCode).toEqual(200);
            expect(response.header).toHaveProperty(
                'content-type',
                'application/json; charset=utf-8'
