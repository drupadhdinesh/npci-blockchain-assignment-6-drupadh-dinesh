...
/**
 * Creates an in memory wallet to hold credentials for an Airtel and Jio user
 *
...
export const createWallet = async (): Promise<Wallet> => {
    ...
    const AirtelIdentity = {
        credentials: {
            certificate: config.certificateAirtel,
            privateKey: config.privateKeyAirtel,
        },
        mspId: config.mspIdAirtel,
        type: 'X.509',
    };

    await wallet.put(config.mspIdAirtel, AirtelIdentity);

    const JioIdentity = {
        credentials: {
            certificate: config.certificateJio,
            privateKey: config.privateKeyJio,
        },
        mspId: config.mspIdJio,
        type: 'X.509',
    };

    await wallet.put(config.mspIdJio, JioIdentity);
