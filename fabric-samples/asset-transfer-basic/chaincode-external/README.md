...
To set the path within the peer container, you will need to modify the docker compose file to mount a couple of additional volumes. Open the file `test-network/compose/docker/docker-compose-test-net.yaml`, and add to the `volumes` section of both `peer0.Airtel.example.com` and `peer0.Jio.example.com` the following two lines:
...
Run the following commands to install the `asset-transfer-basic-external.tar.gz` chaincode on Airtel. The `setGlobals` function simply sets the environment variables that allow you to act as Airtel or Jio.
...
Install the chaincode package on the Jio peer:
...
```
setGlobals 1
peer lifecycle chaincode queryinstalled --peerAddresses localhost:7051 --tlsRootCertFiles organizations/peerOrganizations/Airtel.example.com/peers/peer0.Airtel.example.com/tls/ca.crt
```
...
```
docker run -it --rm --name asset-transfer-basic.Airtel.example.com --hostname asset-transfer-basic.Airtel.example.com --env-file chaincode.env --network=fabric_test hyperledger/asset-transfer-basic
```
...
peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$PWD/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" --channelID mychannel --name basic --peerAddresses localhost:7051 --tlsRootCertFiles "$PWD/organizations/peerOrganizations/Airtel.example.com/peers/peer0.Airtel.example.com/tls/ca.crt" --peerAddresses localhost:9051 --tlsRootCertFiles organizations/peerOrganizations/Jio.example.com/peers/peer0.Jio.example.com/tls/ca.crt --version 1.0 --sequence 1
```
...
For `Airtel.example.com`

```
openssl req -nodes -x509 -newkey rsa:4096 -keyout crypto/key1.pem -out crypto/cert1.pem -subj "/C=IN/ST=KA/L=Bangalore/O=example Inc/OU=Developer/CN=asset-transfer-basic.Airtel.example.com/emailAddress=dev@asset-transfer-basic.Airtel.example.com"
```

For `Jio.example.com`

```
openssl req -nodes -x509 -newkey rsa:4096 -keyout crypto/key2.pem -out crypto/cert2.pem -subj "/C=IN/ST=KA/L=Bangalore/O=example Inc/OU=Developer/CN=asset-transfer-basic.Jio.example.com/emailAddress=dev@asset-transfer-basic.Jio.example.com"
```

- Copy the CA file contents for both `Airtel.example.com` & `Jio.example.com`

```
cp ../../test-network/organizations/peerOrganizations/Airtel.example.com/ca/ca.Airtel.example.com-cert.pem crypto/rootcert1.pem
cp ../../test-network/organizations/peerOrganizations/Jio.example.com/ca/ca.Jio.example.com-cert.pem crypto/rootcert2.pem
```
...
```
{
  "address": "asset-transfer-basic.Airtel.example.com:9999",
  "dial_timeout": "10s",
  "tls_required": true,
...
- Follow the instructions in [Package](#packaging-and-installing-chaincode) and [Install](#installing-the-external-chaincode) steps for each organization. Remember that the chaincode server's address for the second organization is `asset-transfer-basic.Jio.example.com:9999`.
