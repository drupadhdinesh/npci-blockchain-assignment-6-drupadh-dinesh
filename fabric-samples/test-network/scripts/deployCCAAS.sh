...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    set -x
    ${CONTAINER_CLI} run --rm -d --name peer0Airtel_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    ${CONTAINER_CLI} run  --rm -d --name peer0Jio_${CC_NAME}_ccaas \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
    ...
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0Airtel_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0Jio_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
...
## Install chaincode on peer0.Airtel and peer0.Jio
infoln "Installing chaincode on peer0.Airtel..."
installChaincode 1
infoln "Install chaincode on peer0.Jio..."
installChaincode 2
...
## approve the definition for Airtel
approveForMyOrg 1

## check whether the chaincode definition is ready to be committed
## expect Airtel to have approved and Jio not to
checkCommitReadiness 1 "\"AirtelMSP\": true" "\"JioMSP\": false"
checkCommitReadiness 2 "\"AirtelMSP\": true" "\"JioMSP\": false"

## now approve also for Jio
approveForMyOrg 2

## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness 1 "\"AirtelMSP\": true" "\"JioMSP\": true"
checkCommitReadiness 2 "\"AirtelMSP\": true" "\"JioMSP\": true"
