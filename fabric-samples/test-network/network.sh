...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.Airtel.example.com Airtel
    peer_cert admin Admin@Airtel.example.com Airtel

    infoln "Creating Jio Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.Jio.example.com Jio
    peer_cert admin Admin@Jio.example.com Jio
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/Airtel/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for Airtel and Jio"
...
# call the script to create the channel, join the peers of Airtel and Jio,
...
function networkDown() {
  ...
  # stop org3 containers also in addition to Airtel and Jio, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.Airtel.example.com docker_peer0.Jio.example.com
