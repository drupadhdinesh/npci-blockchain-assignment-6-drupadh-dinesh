...
In this example, there are two organizations (org). We will register new identities using the Airtel and Jio Certificate Authorities (CA's), and then use the CA's to generate each identity's certificate and private key.
...
The terminal we have been using will represent Airtel. We will use the Airtel CA to create a new identity. Set the Fabric CA client home to the MSP of the Airtel CA admin (this identity was generated by the test network script).

```bash
export FABRIC_CA_CLIENT_HOME=${PWD}/organizations/peerOrganizations/Airtel.example.com/
```

Register `Person1` identity to Airtel.
```bash
fabric-ca-client register --caname ca-Airtel --id.name person1 --id.secret person1pw --id.type client --tls.certfiles "${PWD}/organizations/fabric-ca/Airtel/tls-cert.pem"
```

Generate the identity certificates and MSP folder.

```bash
fabric-ca-client enroll -u https://person1:person1pw@localhost:7054 --caname ca-Airtel -M "${PWD}/organizations/peerOrganizations/Airtel.example.com/users/person1@Airtel.example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/Airtel/tls-cert.pem"
```

Copy the Node OU configuration file into the identity MSP folder.

```bash
cp "${PWD}/organizations/peerOrganizations/Airtel.example.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/Airtel.example.com/users/person1@Airtel.example.com/msp/config.yaml"
```

Open a new terminal to represent Jio and navigate to fabric-samples/test-network. We'll use the Jio CA to create the Jio identities. Set the Fabric CA client home to the MSP of the Jio CA admin.
...
Register `Person2`, `Person3`, `Person4`, `Person5` to Jio.

```bash
fabric-ca-client register --caname ca-Jio --id.name person2 --id.secret person2pw --id.type client --tls.certfiles "${PWD}/organizations/fabric-ca/Jio/tls-cert.pem"
fabric-ca-client enroll -u https://person2:person2pw@localhost:8054 --caname ca-Jio -M "${PWD}/organizations/peerOrganizations/Jio.example.com/users/person2@Jio.example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/Jio/tls-cert.pem"
cp "${PWD}/organizations/peerOrganizations/Jio.example.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/Jio.example.com/users/person2@Jio.example.com/msp/config.yaml"

fabric-ca-client register --caname ca-Jio --id.name person3 --id.secret person3pw --id.type client --tls.certfiles "${PWD}/organizations/fabric-ca/Jio/tls-cert.pem"
fabric-ca-client enroll -u https://person3:person3pw@localhost:8054 --caname ca-Jio -M "${PWD}/organizations/peerOrganizations/Jio.example.com/users/person3@Jio.example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/Jio/tls-cert.pem"
cp "${PWD}/organizations/peerOrganizations/Jio.example.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/Jio.example.com/users/person3@Jio.example.com/msp/config.yaml"

fabric-ca-client register --caname ca-Jio --id.name person4 --id.secret person4pw --id.type client --tls.certfiles "${PWD}/organizations/fabric-ca/Jio/tls-cert.pem"
fabric-ca-client enroll -u https://person4:person4pw@localhost:8054 --caname ca-Jio -M "${PWD}/organizations/peerOrganizations/Jio.example.com/users/person4@Jio.example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/Jio/tls-cert.pem"
cp "${PWD}/organizations/peerOrganizations/Jio.example.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/Jio.example.com/users/person4@Jio.example.com/msp/config.yaml"

fabric-ca-client register --caname ca-Jio --id.name person5 --id.secret person5pw --id.type client --tls.certfiles "${PWD}/organizations/fabric-ca/Jio/tls-cert.pem"
fabric-ca-client enroll -u https://person5:person5pw@localhost:8054 --caname ca-Jio -M "${PWD}/organizations/peerOrganizations/Jio.example.com/users/person5@Jio.example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/Jio/tls-cert.pem"
cp "${PWD}/organizations/peerOrganizations/Jio.example.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/Jio.example.com/users/person5@Jio.example.com/msp/config.yaml"
```
...
export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/Airtel.example.com/users/person1@Airtel.example.com/msp
...
```
x509::CN=person1,OU=client,O=Hyperledger,ST=North Carolina,C=US::CN=ca.Airtel.example.com,O=Airtel.example.com,L=Durham,ST=North Carolina,C=US
```
...
Switch to the Jio terminal and set the following environment variables.
```bash
export FABRIC_CFG_PATH=$PWD/../config/
export CORE_PEER_TLS_ENABLED=true
export CORE_PEER_LOCALMSPID=JioMSP
export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/Jio.example.com/users/person2@Jio.example.com/msp
