...
function chaincode_command_group() {
  ...
  if [ "${COMMAND}" == "deploy" ]; then
    ...
    log "Installing chaincode for Airtel"
    install_chaincode $@
    log "🏁 - Chaincode is installed"

  elif [ "${COMMAND}" == "approve" ]; then
    log "Approving chaincode for Airtel"
    approve_chaincode $@
    log "🏁 - Chaincode is approved"

  elif [ "${COMMAND}" == "commit" ]; then
    log "Committing chaincode for Airtel"
    commit_chaincode $@
...
function query_chaincode() {
  ...
  export_peer_context Airtel peer1
...
function query_chaincode_metadata() {
  ...
  log ''
  log 'Airtel-Peer2:'
  export_peer_context Airtel peer2
  peer chaincode query -n $cc_name -C $CHANNEL_NAME -c $args
}
...
function launch_chaincode_service() {
  ...
  cat kube/${org}/${org}-cc-template.yaml \
    ...
    | exec kubectl -n $Airtel_NS apply -f -

  kubectl -n $Airtel_NS rollout status deploy/${org}${peer}-ccaas-${cc_name}
