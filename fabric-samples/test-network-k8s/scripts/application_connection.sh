...
function construct_application_configmap() {
  ...
  local peer_pem=$CHANNEL_MSP_DIR/peerOrganizations/Airtel/msp/tlscacerts/tlsca-signcert.pem
  local ca_pem=$CHANNEL_MSP_DIR/peerOrganizations/Airtel/msp/cacerts/ca-signcert.pem

  echo "$(json_ccp 1 $peer_pem $ca_pem)" > build/application/gateways/Airtel_ccp.json

  peer_pem=$CHANNEL_MSP_DIR/peerOrganizations/Jio/msp/tlscacerts/tlsca-signcert.pem
  ca_pem=$CHANNEL_MSP_DIR/peerOrganizations/Jio/msp/cacerts/ca-signcert.pem

  echo "$(json_ccp 2 $peer_pem $ca_pem)" > build/application/gateways/Jio_ccp.json

  pop_fn

  push_fn "Getting Application Identities"

  local cert=$ENROLLMENT_DIR/Airtel/users/Airteladmin/msp/signcerts/cert.pem
  local pk=$ENROLLMENT_DIR/Airtel/users/Airteladmin/msp/keystore/key.pem

  echo "$(app_id AirtelMSP $cert $pk)" > build/application/wallet/appuser_Airtel.id

  local cert=$ENROLLMENT_DIR/Jio/users/Jioadmin/msp/signcerts/cert.pem
  local pk=$ENROLLMENT_DIR/Jio/users/Jioadmin/msp/keystore/key.pem

  echo "$(app_id JioMSP $cert $pk)" > build/application/wallet/appuser_Jio.id
  ...
  kubectl -n $NS create configmap app-fabric-tls-v1-map --from-file=$CHANNEL_MSP_DIR/peerOrganizations/Airtel/msp/tlscacerts
  pop_fn
  ...
  push_fn "Creating ConfigMap \"app-fabric-Airtel-v1-map\" with Organization 1 information for the application"

cat <<EOF > build/app-fabric-Airtel-v1-map.yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: app-fabric-Airtel-v1-map
data:
  fabric_channel: ${CHANNEL_NAME}
  fabric_contract: ${CHAINCODE_NAME}
  fabric_wallet_dir: /fabric/application/wallet
  fabric_gateway_hostport: Airtel-peer-gateway-svc:7051
  fabric_gateway_sslHostOverride: Airtel-peer-gateway-svc
  fabric_user: appuser_Airtel
  fabric_gateway_tlsCertPath: /fabric/tlscacerts/tlsca-signcert.pem
EOF

  kubectl -n $NS apply -f build/app-fabric-Airtel-v1-map.yaml
