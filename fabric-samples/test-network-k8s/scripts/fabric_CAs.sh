...
function launch_ECert_CAs() {
  ...
  apply_template kube/Airtel/Airtel-ca.yaml $Airtel_NS
  apply_template kube/Jio/Jio-ca.yaml $Jio_NS

  kubectl -n $ORG0_NS rollout status deploy/org0-ca
  kubectl -n $Airtel_NS rollout status deploy/Airtel-ca
  kubectl -n $Jio_NS rollout status deploy/Jio-ca
...
function init_tls_cert_issuers() {
  ...
  kubectl -n $Airtel_NS apply -f kube/root-tls-cert-issuer.yaml
  kubectl -n $Airtel_NS wait --timeout=30s --for=condition=Ready issuer/root-tls-cert-issuer
  kubectl -n $Jio_NS apply -f kube/root-tls-cert-issuer.yaml
  kubectl -n $Jio_NS wait --timeout=30s --for=condition=Ready issuer/root-tls-cert-issuer
  ...
  kubectl -n $Airtel_NS apply -f kube/Airtel/Airtel-tls-cert-issuer.yaml
  kubectl -n $Jio_NS apply -f kube/Jio/Jio-tls-cert-issuer.yaml

  kubectl -n $ORG0_NS wait --timeout=30s --for=condition=Ready issuer/org0-tls-cert-issuer
  kubectl -n $Airtel_NS wait --timeout=30s --for=condition=Ready issuer/Airtel-tls-cert-issuer
  kubectl -n $Jio_NS wait --timeout=30s --for=condition=Ready issuer/Jio-tls-cert-issuer
...
function enroll_bootstrap_ECert_CA_users() {
  ...
  enroll_bootstrap_ECert_CA_user Airtel $Airtel_NS
  enroll_bootstrap_ECert_CA_user Jio $Jio_NS
